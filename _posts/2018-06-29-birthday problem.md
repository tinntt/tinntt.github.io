...
<p align="justify">
	đặt n = 365, (số ngày trong năm).
		k là số học sinh cần phải có.
		b<sub>i</sub> là ngày sinh của học sinh i.
		với từng cặp học sinh (i,j) ta đặt 1 biến X<sub>ij</sub> với 1 &le; i < j &le; k: 
		X<sub>ij</sub> = I{xác suất để học sinh i và j trùng ngày sinh} = 1 (nếu trùng) = 0 (nếu không trùng).

	Đầu tiên, xác suất để 1 học sinh rơi vào 1 ngày a nào đó là 1/n.
	Pr{b<sub>i</sub> = a} = 1/n.
	=> Pr{b<sub>i</sub> = a và b<sub>j</sub> = a} = Pr{b<sub>i</sub> = a}Pr{b<sub>j</sub> = a} = 1/n<sup>2</sup> (2 xác suất độc lập).

	1 năm có n ngày, vậy xác suất để 2 học sinh trùng ngày sinh vào 1 ngày bất kì là:
	&sum;<sub>a=1</sub><sup>n</sup> (1/n<sup>2</sup>) = n(1/n<sup>2</sup>) = 1/n.

	Vậy sự kiện để 2 học sinh có cùng ngày sinh là E[X<sub>ij</sub>] = Pr{2 học sinh có cùng ngày sinh} = 1/n.

	Đặt X là tổng số các cặp học sinh có cùng ngày sinh với nhau:

	X = &sum;<sub>i=1</sub><sup>k</sup>&sum;<sub>j=i+1</sub><sup>k</sup>X<sub>ij</sub>.

	vậy ta có E[X] = E[&sum;<sub>i=1</sub><sup>k</sup>&sum;<sub>j=i+1</sub><sup>k</sup>X<sub>ij</sub>]
				   = &sum;<sub>i=1</sub><sup>k</sup>&sum;<sub>j=i+1</sub><sup>k</sup>E[X<sub>ij</sub>]
				   = C(k,2)1/n ( C(k,2) = chọn 2 trong k học sinh).
				   = k(k-1)/2n.
